...
void Mqtt_client::setup() {
  if (mIsSetup == false)
  {
    Logger::Instance().write(LogLevel::INFO, FPSTR("Setting up Mqtt_client"));
    ...
    Logger::Instance().write(LogLevel::INFO, FPSTR("Finished setting up Mqtt_client"));
    mIsSetup = true;
  }
}
...
void Mqtt_client::loop()
{
  if (enabled())
  {
    Logger::Instance().write(LogLevel::DEBUG, FPSTR("Mqtt_client::loop"));
