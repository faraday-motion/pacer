...
public:
  ...
  static Modulebase * getModuleInstance(byte id, int configuration, IFMV * fmv)
  {
      ...
      case Configurations::MQTT_CONNECTION_CONFIG :
        Logger::Instance().write(LogLevel::INFO, FPSTR("getModuleInstance MQTT_CONNECTION_CONFIG"));
        Logger::Instance().write(LogLevel::INFO, FPSTR("Free Heap: "), String(ESP.getFreeHeap()));
        mod = new Mqtt_connection(id, fmv);
        break;
      case Configurations::MQTT_CONTROL_CONFIG :
        Logger::Instance().write(LogLevel::INFO, FPSTR("getModuleInstance MQTT_CONTROL_CONFIG"));
        Logger::Instance().write(LogLevel::INFO, FPSTR("Free Heap: "), String(ESP.getFreeHeap()));
        mod = new Mqtt_control(id, fmv);
        break;
